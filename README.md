...
第一箱是仅含 3 辆玩具车的短教学；3 辆车会错峰轻微缩放，并持续显示“点击车模型”提示直到玩家首次选择。玩具先以 3D 模型飞入收纳槽，再切换为渲染图标并在三消时聚拢爆开；第二箱起进入随机牌库、朝向和物理堆叠的正式关卡。

在线游玩：https://softxuep-dotcom.github.io/Toybox-Trio/

## 运行

修复主角玩具时，每组三消会让分离的模型部件逐步归位。前四箱规模为 3 / 12 / 24 / 27 件，第五箱起为 54 件。开始、暂停和通关面板中的“我的玩具架”保留已修复玩具，支持点击跳动动画；收藏记录保存在当前浏览器，集齐五款会点亮整排玩具架。
